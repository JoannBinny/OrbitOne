...
def build_tools(organization_id: int, run_id: int = None) -> list:
    """Build the agent's tools bound to one real run's organization/run id.

    organization_id and run_id are closed over here instead of being
    LLM-supplied tool arguments — the model previously had to correctly copy
    an organization id into every tool call, and sometimes didn't. Injecting
    them in Python is strictly more reliable and keeps every tool call scoped
    to the correct organization and (where relevant) the correct AgentRun.
    """
    ...
    def get_pending_approvals() -> list:
        ...
        resp = requests.get(
            f"{BACKEND_URL}/approvals",
            params={"status": "pending", "organization_id": organization_id},
            timeout=10,
        )
        resp.raise_for_status()
        return resp.json()
